...
public://定义接口
	...
	// 3.2 定义查询接口处理
	ENDPOINT(API_M_GET, "/safety/admin-password", queryAdminPwd, QUERY(String, xid), API_HANDLER_AUTH_PARAME) {
	...
	// 3.2 定义修改接口处理
	API_HANDLER_ENDPOINT_AUTH(API_M_PUT, "/safety/admin-password", modifyAdminPwd, BODY_DTO(AdminPwdDTO::Wrapper, dto), executeModifyAdminPwd(dto));

private:
	//执行获取管理员密码
	AdminPwdJsonVO::Wrapper executeQueryAdminPwd(const String& xid);
	//执行修改管理员密码
	Uint64JsonVO::Wrapper executeModifyAdminPwd(const AdminPwdDTO::Wrapper& dto);
};
