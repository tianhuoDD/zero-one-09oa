...
public:
	...
	// 定义端点
	API_HANDLER_ENDPOINT(API_M_POST, "/c1-org/file/upload", uploadFile, REQUEST(std::shared_ptr<IncomingRequest>, request), execUploadOne(request));

	// 定义一个多文件上传接口
	// 定义描述
	API_DEF_ENDPOINT_INFO(ZH_WORDS_GETTER("file.upload-more.summary"), uploadFileMore, StringJsonVO::Wrapper);
	// 定义端点
	API_HANDLER_ENDPOINT(API_M_POST, "/c1-org/file/upload-more", uploadFileMore, REQUEST(std::shared_ptr<IncomingRequest>, request), execUploadMore(request));
	...
	// 定义端点
	ENDPOINT(API_M_GET, "/c1-org/file/download", downloadFile, QUERY(String, filename)) {
	...
	// 定义端点
	ENDPOINT(API_M_GET, "/c1-org/file/**", htmlFile, REQUEST(std::shared_ptr<IncomingRequest>, request)) {
	...
	// 定义端点
	API_HANDLER_ENDPOINT(API_M_POST, "/c1-org/file/editor-upload-image", uploadImage, REQUEST(std::shared_ptr<IncomingRequest>, request), execUploadEditorImage(request));
private: // 定义接口执行函数
	// 执行单文件上传处理
