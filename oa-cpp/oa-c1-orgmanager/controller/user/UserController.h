...
public:
	...
	// 定义查询所有用户信息接口端点处理
	API_HANDLER_ENDPOINT_QUERY_AUTH(API_M_GET, "/c1-org/user/query-all", queryAllUser, UserQuery, executeQueryAll(query));
	...
	ENDPOINT(API_M_POST, "/c1-org/user/modify-user", modifyUser, BODY_STRING(String, fileBody), QUERIES(QueryParams, qps), API_HANDLER_AUTH_PARAME) {
		// 解析查询参数
	...
	// 定义查询用户菜单接口端点处理
	API_HANDLER_ENDPOINT_NOPARAM_AUTH(API_M_GET, "/c1-org/user/query-menu", queryMenu, executeQueryMenu(authObject->getPayload()))
private:
	// 查询所有
