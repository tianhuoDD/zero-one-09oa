...
bool getStartArg(int argc, char* argv[]) {
	...
	std::string dbHost = "192.168.26.135";
	int dbPort = 3306;
	int dbMax = 5;
#ifdef LINUX
	// Nacos配置参数
	std::string nacosAddr = "192.168.26.135:8848";
...
//定义
void testDfs() {
	FastDfsClient dfs("192.168.79.129");
	std::string field =dfs.uploadFile("D:\\01ball\\image\\1.jpg");
	std::cout << field<<endl;
	bool isDel = dfs.deleteFile("group1/M00/00/00/oYYBAGcaVjyAHUvCAAI7but4cJs559.jpg");
	std::cout << isDel << endl;
...
void testExcel() {
	...
	excel.writeVectorToFile("D:/public/excel/1.xlsx", "test", data);
	//写到fastdfs
	auto buff = excel.writeVectorToBuff("test", data);
	FastDfsClient dfs("192.168.79.129");
	auto field=dfs.uploadFile(reinterpret_cast<const char*>(buff.data()), buff.size(),"xlsx");
	std::cout << field<<std::endl;
	
	
	//读excel
	data = ExcelComponent::readIntoVector("D:/public/excel/1.xlsx", "test");
