		...
		<div style="display: flex">
			...
			<div style="flex: 1">
				<el-card shadow="never">
					...
					<el-table :data="selectedRoles" style="width: 100%" height="300">
...
const handleCheck = (data) => {
	if (!props.isMultiple) {
		// 在树形组件中只保留当前选中的节点，取消其他节点的选中状态
		if (treeRef.value) {
			const checkedNodes = treeRef.value.getCheckedNodes(true);
			selectedRoles.value = checkedNodes.map((node) => ({
				id: node.id,
				label: node.label,
			}));
			checkedNodes.forEach((node) => {
				if (node.id !== data.id) {
					...
					// 从 selectedRoles 中移除对应的项
					selectedRoles.value = selectedRoles.value.filter((role) => role.id !== node.id);
				}
			});
		}
	} else {
